...
const MAX_NODES_PER_AXIS = 16;
const CLIENT_TTL_MS = 14 * 60_000; // un poco menos que el TTL del servidor (15 min)
...
export class WeatherGridCache {
  ...
  chooseStep(bbox: [number, number, number, number]): number {
    ...
    for (const step of STEP_LADDER) {
      if (span / step <= MAX_NODES_PER_AXIS) return step;
    }
  ...
  private nodesForBBox(bbox: [number, number, number, number], step: number) {
    const [w, s, e, n] = bbox;
    const ixMin = Math.floor(w / step), ixMax = Math.ceil(e / step);
    const iyMin = Math.floor(s / step), iyMax = Math.ceil(n / step);
   ...
   *  cacheados y vigentes, pidiendo solo lo que falta. Devuelve el espaciado
   *  usado y el rango de índices, para que quien renderiza arme una ventana
   *  de muestreo densa. */
  async ensureCoverage(bbox: [number, number, number, number]): Promise<{
    step: number;
    ixRange: [number, number];
    iyRange: [number, number];
  }> {
    const step = this.chooseStep(bbox);
   ...
   *  está cacheado (el renderer lo trata como transparente). */
