...
export const WeatherService = {
  ...
  // NUEVO MÉTODO PARA CONSULTAR MÚLTIPLES PUNTOS EN UNA SOLA LLAMADA
  async points(
...
export const FireRiskService = {
  compute(v: {
    ...
    if (score < 0.30)
      return { score, label: "Muy Bajo", color: "#22c55e" };
    if (score < 0.50)
      return { score, label: "Bajo", color: "#facc15" };
    if (score < 0.70)
      return { score, label: "Moderado", color: "#f97316" };
    if (score < 0.90)
      return { score, label: "Alto", color: "#dc2626" };
